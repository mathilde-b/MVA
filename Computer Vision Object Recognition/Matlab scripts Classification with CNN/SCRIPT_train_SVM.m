...
%         C = 10 ;
        [w, bias] = trainLinearSVM(histograms, labels, C(ii)) ;
        ...
        for i=1:size(index_test,1)
            testResults(i)=mean(testScores(index_test(i,1):index_test(i,2)));
            testTruth(i)=testLabels(index_test(i,1));
        end
        result(ii,j)=sum((abs(sign(testResults)+testTruth))/2)/length(testTruth);
    end
end
